...
import { CardknoxApi } from "./cardknoxapi";
...
Meteor.methods({
   ...
   */
  "cardknoxSubmit"(transactionType, cardData, paymentData) {
   ...
   */
  "cardknox/refund/create"(paymentMethod, amount) {
    check(paymentMethod, Reaction.Schemas.PaymentMethod);
    check(amount, Number);
    const { transactionId } = paymentMethod;
    const response = CardknoxApi.methods.refund.call({
      transactionId,
      amount
    });
    const results = {
   ...
   * @return {Object} result
   */
  "cardknox/refund/list"(paymentMethod) {
    check(paymentMethod, Reaction.Schemas.PaymentMethod);
    const { transactionId } = paymentMethod;
    const response = CardknoxApi.methods.refunds.call({
      transactionId
    });
    const result = [];
