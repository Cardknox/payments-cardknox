...
import { CardknoxApi, RISKY_TEST_CARD } from "./cardknoxapi";

const paymentMethod = {
  processor: "Generic",
  storedCard: "Visa 4242",
  paymentPackageId: "vrXutd72c2m7Lenqw",
  paymentSettingsKey: "cardknox-paymentmethod",
  status: "captured",
  mode: "authorize",
  createdAt: new Date()
};
...
describe("CardknoxApi", function () {
  ...
  it("should return data from ThirdPartyAPI authorize", function () {
    ...
    const transaction = CardknoxApi.methods.authorize.call({
      transactionType,
      cardData,
      paymentData
    });
  ...
  it("should return data from ThirdPartAPI capture", function (done) {
    ...
    const amount = 19.99;
    const results = CardknoxApi.methods.capture.call({
      authorizationId,
      amount
    });
...
describe("Submit payment", function () {
  ...
  it("should call Cardknox API with card and payment data", function () {
    ...
    const authorizeStub = sandbox.stub(CardknoxApi.methods.authorize, "call", () => authorizeResult);
    const results = Meteor.call("cardknoxSubmit", "authorize", cardData, paymentData);
  ...
  it("should throw an error if card data is not correct", function () {
    ...
    expect(function () {
      Meteor.call("cardknoxSubmit", "authorize", badCardData, paymentData);
    }).to.throw;
  });
});
...
describe("Capture payment", function () {
  ...
  it("should call CardknoxApi with transaction ID", function () {
    ...
    const captureStub = sandbox.stub(CardknoxApi.methods.capture, "call", () => captureResults);
    const results = Meteor.call("cardknox/payment/capture", paymentMethod);
  ...
  it("should throw an error if transaction ID is not found", function () {
    sandbox.stub(CardknoxApi.methods, "capture", function () {
      throw new Meteor.Error("not-found", "Not Found");
    });
    expect(function () {
      Meteor.call("cardknox/payment/capture", "abc123");
    }).to.throw;
  });
});
...
describe("Refund", function () {
  ...
  it("should call CardknoxApi with transaction ID", function () {
    ...
    const refundStub = sandbox.stub(CardknoxApi.methods.refund, "call", () => refundResults);
    Meteor.call("cardknox/refund/create", paymentMethod, amount);
  ...
  it("should throw an error if transaction ID is not found", function () {
    sandbox.stub(CardknoxApi.methods.refund, "call", function () {
      throw new Meteor.Error("not-found", "Not Found");
    });
    const transactionId = "abc1234";
    paymentMethod.transactionId =  transactionId;
    expect(function () {
      Meteor.call("cardknox/refund/create", paymentMethod, 19.99);
...
describe("List Refunds", function () {
  ...
  it("should call CardknoxApi with transaction ID", function () {
    ...
    const refundStub = sandbox.stub(CardknoxApi.methods.refunds, "call", () => refundResults);
    Meteor.call("cardknox/refund/list", paymentMethod);
    expect(refundStub).to.have.been.calledWith(refundArgs);
  });

  it("should throw an error if transaction ID is not found", function () {
    sandbox.stub(CardknoxApi.methods, "refunds", function () {
      throw new Meteor.Error("not-found", "Not Found");
    });
    expect(() => Meteor.call("cardknox/refund/list", paymentMethod)).to.throw(Meteor.Error, /Not Found/);
  });
});
