...
import { Cardknox } from "../../lib/api";
import { CardknoxPayment } from "../../lib/collections/schemas";
...
function handleCardknoxSubmitError(error) {
...
Template.cardknoxPaymentForm.helpers({
  CardknoxPayment() {
    return CardknoxPayment;
  }
});

AutoForm.addHooks("cardknox-payment-form", {
  onSubmit(doc) {
    submitting = true;
    ...
    Cardknox.authorize(form, {
      ...
      let paymentMethod;
      if (error) {
        handleCardknoxSubmitError(error);
        ...
        if (transaction.saved === true) {
          ...
          handleCardknoxSubmitError(transaction.error);
          uiEnd(template, "Resubmit payment");
        }
      }
    });
